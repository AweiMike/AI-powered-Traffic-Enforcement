...
def create_changelog():
    ...
    本次更新內容：
    - ★視覺設計全面改版：主題色由動森葉綠改為低疲勞專業深藍，減輕長時間使用眼睛疲勞
    - ★地圖派出所篩選重新設計：改為 Checkbox 列表並顯示該單位事故/違規件數，依件數自動排序
    - 地圖視覺化：新增派出所/單位複選篩選，可查看特定轄區熱點
    - 地圖視覺化：新增範圍圈選工具，動態顯示圈內事故種類、件數與單位分布
    - 地圖視覺化：事故/違規點位新增「時間」顯示
    - 地圖視覺化：違規點位新增「舉發類型」顯示
    - 綜合執法成效：違規案件新增攔停/逕行舉發細分比較
    - 資料匯入：支援回補既有資料的舉發類型欄位與轄區派出所
    - 資料匯入：支援瀏覽器批次上傳多檔案
