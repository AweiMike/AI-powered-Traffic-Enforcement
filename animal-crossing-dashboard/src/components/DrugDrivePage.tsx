      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-gradient-to-br from-pink-50 to-pink-100/50 border border-pink-200 rounded-2xl p-4">
              ...
              <p className="text-[10px] text-pink-400/80 mt-1">§35 I-2 吸食毒品 + IV 毒品拒檢</p>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl nook-shadow overflow-hidden mb-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-pink-50 text-nook-text/80">
                  ...
                  <tbody>
                    {data.unit_group_ranking.map((g: any, i: number) => (
                      <tr key={g.group} className={i % 2 === 0 ? 'bg-white' : 'bg-pink-50/30'}>
                        <td className="px-4 py-2.5 text-center">
                          <RankBadge rank={g.rank} />
                        </td>
                        <td className="px-4 py-2.5 font-medium text-nook-text">{g.group}</td>
                        <td className="px-4 py-2.5 text-right font-bold text-pink-700 tabular-nums">{g.tickets}</td>
                  ...
                  <div key={d.district} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="tabular-nums">
                        <span className="font-bold text-pink-700">{d.tickets}</span>
                        {d.crash > 0 && <span className="text-red-600 ml-1">（肇事 {d.crash}）</span>}
                      </span>
                    </div>
                    <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-pink-400 to-pink-600 rounded-full"
                ...
                {data.by_shift?.length > 0 ? data.by_shift.map((s: any) => (
                  <div key={s.shift_id} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="tabular-nums">
                        <span className="font-bold text-pink-700">{s.tickets}</span>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.rows.map((row: any, i: number) => (
                      <tr key={row.unit} className={i % 2 === 0 ? 'bg-white' : 'bg-pink-50/30'}>
                        <td className="px-4 py-2.5 font-medium text-nook-text">{row.unit}</td>
                        <td className="px-4 py-2.5 text-right font-bold text-pink-700 tabular-nums">{row.tickets}</td>
