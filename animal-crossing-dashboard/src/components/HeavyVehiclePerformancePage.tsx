...
const HeavyVehiclePerformancePage: React.FC = () => {
  ...
  return (
    <div className="p-8">
      ...
      {/* 大型車週趨勢 */}
      <React.Suspense fallback={<TrendCardSkeleton title="大型車週趨勢與專業判讀" />}>
        <TrendCard
          range={range}
          title="大型車週趨勢與專業判讀"
      ...
      {/* 大型車事故特徵剖析（年齡/性別/車種/事故型態/保護裝備/駕照狀態/班別/路線） */}
      ...
      <React.Suspense fallback={<div className="bg-white/80 rounded-2xl p-6 nook-shadow h-40 animate-pulse mb-6" />}>
        <SignalCheckCard range={range} topic="heavy" />
      </React.Suspense>
      <ProfileCard topic="heavy" title="大型車事故特徵剖析" range={range} />
