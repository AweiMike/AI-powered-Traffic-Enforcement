...
def evaluate_improvement(db: Session, imp: Improvement, days: int = 365) -> dict:
    """
    計算單筆改善措施登記的 before-after 成效評估（抽取自 /evaluations 端點核心邏輯，
    行為與端點內原邏輯完全相同），供其他模組（如 AI 報告 analytics_engine）共用，
    避免同一套評估邏輯維護兩份。

    回傳格式與 /evaluations 端點回傳的單筆 item 完全相同：
    - 尚無 after 期資料（含資料庫完全無事故資料）時：{"status": "no_after_data", ...}
    - 否則含 before/after/pct_change/baseline_pct/net_pct/preliminary/verdict
    """
    base = {
        "id": imp.id,
        "title": imp.title,
        "measure_type": imp.measure_type,
        "implemented_date": imp.implemented_date.isoformat(),
    ...
    if data_end is None:
        return {**base, "status": "no_after_data"}

    impl = imp.implemented_date
    ...
    # === 點位（半徑內）統計 ===
    before_crashes = _crashes_in_radius(db, imp.latitude, imp.longitude, imp.radius_m, before_start, before_end)
    after_crashes = _crashes_in_radius(db, imp.latitude, imp.longitude, imp.radius_m, impl, after_end)
...
async def get_evaluations(
    ...
    # 核心 before/after 評估邏輯已抽成 module-level 函式 evaluate_improvement()，
    # 供本端點與其他模組（如 AI 報告 analytics_engine）共用（純抽取重構，行為不變）。
    items = [evaluate_improvement(db, r, days) for r in rows]
