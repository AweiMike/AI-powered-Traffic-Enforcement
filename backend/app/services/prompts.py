...
### 5.6 2-30日死亡 → 報告必須註記（警政實務鐵則，不得省略）
- 若 2-30日死亡 >0，報告必須明確註記該情形，說明案件列 A2 但實質有人員亡故
- 僅為註記用途：**不得**因此把該案件改稱為 A1，也**不得**把此人數併入 A1 死亡或死亡統計口徑（死亡統計仍採 24 小時內口徑）
...
- ✅「左鎮分駐所取締比率僅 1.33，為 4 管區最低，執法力道薄弱」
...
{f'- 2-30日死亡：{data.late_deaths} 人（案件分類 A2；死亡統計採24hr口徑不含此數，惟道安檢討須註記）' if data.late_deaths > 0 else ''}
...
- 新化分局實際業務以 4 管區檢討：新化派出所（含那拔）、唪口派出所（含知義）、山上分駐所、左鎮分駐所
...
12. ☐ **若 2-30日死亡 >0，是否已明確註記「案件列 A2 但實質有人員亡故」，且未將此數併入 A1 死亡或死亡統計口徑？**
